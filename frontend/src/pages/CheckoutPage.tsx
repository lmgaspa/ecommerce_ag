...
const pesoPorLivroKg: Record<string, number> = {
  extase: 0.11,
  regressantes: 0.2,
  sempre: 0.096,
};

function calcularFretePorPeso(pesoTotal: number): number {
  if (pesoTotal <= 0.2) return 12;
  if (pesoTotal <= 0.5) return 15;
  if (pesoTotal <= 1) return 20;
  return 25;
}
...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    const cart = getCart(); // chama uma vez
    setCartItems(cart);
    const sum = cart.reduce((acc, item) => acc + item.price * item.quantity, 0);
    setTotal(sum);

    const pesoTotal = cart.reduce((acc, item) => {
      const pesoUnitario = pesoPorLivroKg[item.id] || 0;
      return acc + pesoUnitario * item.quantity;
    }, 0);
    setFrete(pesoTotal > 0 ? calcularFretePorPeso(pesoTotal) : 0);
  }, []);
  ...
  const updateQuantity = (id: string, delta: number) => {
    ...
    const pesoTotal = updated.reduce((acc, item) => {
      const pesoUnitario = pesoPorLivroKg[item.id] || 0;
      return acc + pesoUnitario * item.quantity;
    }, 0);
    setFrete(pesoTotal > 0 ? calcularFretePorPeso(pesoTotal) : 0);
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-4">
      <button
        ...
        className="px-6 py-2 mt-2 bg-green-600 text-white rounded-md shadow-md transition hover:bg-green-700"
      >
        Continuar comprando
      </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="border p-4 rounded space-y-4">
          ...
          <div className="flex justify-between">
            ...
            <span>{frete > 0 ? formatPrice(frete) : "---"}</span>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-600 mt-1">
              Após clicar em "Finalizar compra" você receberá o seu boleto
              bancário. Pode ser impresso ou pago no banco ou online. O pedido
              será confirmado após o pagamento.
            </p>
          ...
          <button className="bg-red-600 text-white py-2 w-full mt-4 rounded">
            Finalizar pedido
          </button>
        </div>
      </div>
